...
RUN CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/bootstrapctl ./cmd/bootstrapctl
...
# base image, which the upstream Misskey (IDENTITY_ORIGIN) HTTPS calls
# need.
FROM gcr.io/distroless/static-debian12:nonroot
COPY --from=builder /out/server /server
COPY --from=builder /out/bootstrapctl /bootstrapctl
