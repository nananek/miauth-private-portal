...
| --- | --- | --- |
| Aria-facing local MiAuth session | Local portal; Aria polls the local check endpoint | Aria supplies an opaque route ID; the server assigns a crypto/rand record ID and state, then binds both to a one-time 10-minute record, callback, `LOCAL_ORIGIN`, requested scopes, and browser/session context |
...
server generates its own unguessable record ID and state with `crypto/rand`,
validates the state, and atomically binds both values to the route record
