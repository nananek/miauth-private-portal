...
void main() {
  test(
    'POST /api/notes/conversation returns the oldest-first ancestor chain',
    () async {
      final misskey = buildTestClient();
      final root = await misskey.notes.create(
        NotesCreateRequest(
          text: 'conversation root ${DateTime.now().microsecondsSinceEpoch}',
        ),
      );
      final child = await misskey.notes.create(
        NotesCreateRequest(
          text: 'conversation child ${DateTime.now().microsecondsSinceEpoch}',
          replyId: root!.id,
        ),
      );
      final grandchild = await misskey.notes.create(
        NotesCreateRequest(
          text:
              'conversation grandchild ${DateTime.now().microsecondsSinceEpoch}',
          replyId: child!.id,
        ),
      );
