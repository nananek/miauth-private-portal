...
void main() {
  test('POST /api/notes/show decodes a previously created note', () async {
    ...
    final shown = await misskey.notes.show(
      NotesShowRequest(noteId: created!.id),
    );
  ...
  test(
    'POST /api/notes/show on an unknown noteId decodes as MisskeyException NO_SUCH_NOTE',
    () async {
      final misskey = buildTestClient();
