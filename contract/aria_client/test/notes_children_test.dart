...
void main() {
  test(
    'POST /api/notes/children returns direct replies oldest-first',
    () async {
      final misskey = buildTestClient();
      final root = await misskey.notes.create(
        NotesCreateRequest(
          text: 'children root ${DateTime.now().microsecondsSinceEpoch}',
        ),
      );
      final first = await misskey.notes.create(
        NotesCreateRequest(
          text: 'children first ${DateTime.now().microsecondsSinceEpoch}',
          replyId: root!.id,
        ),
      );
      final second = await misskey.notes.create(
        NotesCreateRequest(
          text: 'children second ${DateTime.now().microsecondsSinceEpoch}',
          replyId: root.id,
        ),
      );
  ...
  test(
    'POST /api/notes/children on an unknown noteId decodes as MisskeyException NO_SUCH_NOTE',
    () async {
      final misskey = buildTestClient();
