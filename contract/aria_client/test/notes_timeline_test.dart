...
void main() {
  test('POST /api/notes/timeline returns newest-first and paginates with untilId', () async {
    final misskey = buildTestClient();
    await misskey.notes.create(
      NotesCreateRequest(text: 'timeline older ${DateTime.now().microsecondsSinceEpoch}'),
    );
    final newer = await misskey.notes.create(
      NotesCreateRequest(text: 'timeline newer ${DateTime.now().microsecondsSinceEpoch}'),
    );

    final firstPage = (await misskey.notes.homeTimeline(NotesTimelineRequest(limit: 1))).toList();
    ...
    expect(
      ...
      isFalse,
      reason: 'untilId must page to strictly older (or equal-timestamp, tie-broken) notes',
    );
  });

  test('POST /api/notes/timeline with an unknown untilId returns an empty page', () async {
    final misskey = buildTestClient();
